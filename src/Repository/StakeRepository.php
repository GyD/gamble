...
final readonly class StakeRepository implements StakeStore
{
    ...
    public function update(int $id, int $betOptionId, int $contactId, int $amount): Stake
    {
        $statement = $this->pdo->prepare(
            'UPDATE stakes
             SET bet_option_id = :bet_option_id, contact_id = :contact_id, amount = :amount
             WHERE id = :id',
        );
        $statement->execute([
            'id' => $id,
    ...
    {
        $clear = $this->pdo->prepare('UPDATE stakes SET final_payout = NULL WHERE bet_id = :bet_id');
        $clear->execute(['bet_id' => $betId]);
        $update = $this->pdo->prepare('UPDATE stakes SET final_payout = :payout WHERE id = :id AND bet_id = :bet_id');
    ...
    public function findWinnersByBet(int $betId, int $winningOptionId): array
    {
        $statement = $this->pdo->prepare(
            'SELECT stakes.contact_id,
                    ...
                    SUM(stakes.amount) AS winning_stake,
                    SUM(stakes.final_payout) AS payout,
        ...
        return array_map(static fn(array $row): array => [
            ...
            'winning_stake' => (int)$row['winning_stake'],
